...
from ai_parser import parse_file_with_ai, format_today_schedule, format_user_stats, get_user_context
from intent_authority import process_user_input, clear_pending_intent
...
class ChatView(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        # Clarification Options Area (Hidden by default)
        self.options_frame = tk.Frame(self, bg=COLORS["bg"], pady=5)
    ...
    def show_options(self, options):
        ...
        tk.Label(self.options_frame, text="Suggestions:", bg=COLORS["bg"], 
                 fg=COLORS["text_dim"], font=("Segoe UI", 9)).pack(anchor="w")
        
        for opt in options:
            btn = tk.Button(self.options_frame, text=opt, 
                           command=lambda o=opt: self.send_option(o),
                           bg=COLORS["card"], fg=COLORS["accent"], 
                           relief="flat", padx=10, pady=2)
            btn.pack(side="left", padx=5, pady=5)

    def send_option(self, text):
    ...
    def process_ai(self, text, is_file_content=False):
        ...
        try:
            ...
            if action == "clarify":
                q = result.get("question", "Could you clarify?")
                ...
                self.add_msg("assistant", msg)
                
            elif action == "execute" or action == "execute_confirmed":
                ...
                elif self.app.active_view == "routine":
                    self.app.views["routine"].render()
                    
            else:
                self.add_msg("assistant", "I'm not sure what happened.")
    ...
    def handle_result(self, result):
        """Handle file parsing result (legacy support for files)."""
        intent = result.get("intent")
        
        if intent == "schedule_file":
            ...
            self.add_msg("assistant", result.get("message", "Processed file."))
